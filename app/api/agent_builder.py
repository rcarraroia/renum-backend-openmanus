...
async def update_agent(agent_id: UUID4, agent: AgentUpdate, supabase=Depends(get_supabase_client)):
    """
    Atualiza um agente existente.
    """
    try:
        # Verificar se o agente existe
        get_response = supabase.table("agents").select("*").eq("id", str(agent_id)).execute()
...
async def list_tools(category: Optional[str] = None, supabase=Depends(get_supabase_client)):
    """
...
async def update_tool(tool_id: UUID4, tool: ToolUpdate, supabase=Depends(get_supabase_client)):
    """
    Atualiza uma ferramenta existente.
    """
    try:
        # Verificar se a ferramenta existe
        get_response = supabase.table("tools").select("*").eq("id", str(tool_id)).execute()
        ...
        # Atualizar no Supabase
        response = supabase.table("tools").update(update_data).eq("id", str(tool_id)).execute()
...
@router.post("/agent-tools", response_model=AgentTool, status_code=status.HTTP_201_CREATED)
async def create_agent_tool(agent_tool: AgentToolCreate, supabase=Depends(get_supabase_client)):
    """
    Associa uma ferramenta a um agente.
    """
    try:
        # Verificar se o agente existe
        agent_response = supabase.table("agents").select("*").eq("id", str(agent_tool.agent_id)).execute()
        ...
        # Verificar se a ferramenta existe
        tool_response = supabase.table("tools").select("*").eq("id", str(agent_tool.tool_id)).execute()
...
async def list_agent_tools(agent_id: Optional[UUID4] = None, supabase=Depends(get_supabase_client)):
    """
    ...
    try:
        ...
        logger.error(f"Erro ao listar associações entre agentes e ferramentas: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_agent_tool(agent_id: UUID4, tool_id: UUID4, supabase=Depends(get_supabase_client)):
    """
    Obtém uma associação específica entre agente e ferramenta.
    """
    try:
        response = supabase.table("agent_tools").select("*").eq("agent_id", str(agent_id)).eq("tool_id", str(tool_id)).execute()
...
async def update_agent_tool(agent_id: UUID4, tool_id: UUID4, agent_tool: AgentToolUpdate, supabase=Depends(get_supabase_client)):
    """
    Atualiza uma associação existente entre agente e ferramenta.
    """
    try:
        # Verificar se a associação existe
        get_response = supabase.table("agent_tools").select("*").eq("agent_id", str(agent_id)).eq("tool_id", str(tool_id)).execute()
        ...
        # Atualizar no Supabase
        response = supabase.table("agent_tools").update(update_data).eq("agent_id", str(agent_id)).eq("tool_id", str(tool_id)).execute()
        ...
        logger.error(f"Erro ao atualizar associação entre agente e ferramenta: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao atualizar associação entre agente e ferramenta: {str(e)}"
        )

@router.delete("/agent-tools/{agent_id}/{tool_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_agent_tool(agent_id: UUID4, tool_id: UUID4, supabase=Depends(get_supabase_client)):
    """
    ...
    try:
        ...
        # Remover do Supabase
        supabase.table("agent_tools").delete().eq("agent_id", str(agent_id)).eq("tool_id", str(tool_id)).execute()
...
async def create_credential(credential: CredentialCreate, supabase=Depends(get_supabase_client)):
    """
...
async def list_credentials(provider: Optional[str] = None, supabase=Depends(get_supabase_client)):
    """
...
async def get_credential(credential_id: UUID4, supabase=Depends(get_supabase_client)):
    ...
    try:
        ...
        response = supabase.table("credentials").select("*").eq("id", str(credential_id)).eq("user_id", user_id).execute()
...
async def update_credential(credential_id: UUID4, credential: CredentialUpdate, supabase=Depends(get_supabase_client)):
    """
    ...
    try:
        ...
        get_response = supabase.table("credentials").select("*").eq("id", str(credential_id)).eq("user_id", user_id).execute()
        ...
        # Atualizar no Supabase
        response = supabase.table("credentials").update(update_data).eq("id", str(credential_id)).eq("user_id", user_id).execute()
...
async def delete_credential(credential_id: UUID4, supabase=Depends(get_supabase_client)):
    """
    ...
    try:
        ...
        # Remover do Supabase
        supabase.table("credentials").delete().eq("id", str(credential_id)).eq("user_id", user_id).execute()
...
async def get_agent_tools(agent_id: UUID4, supabase=Depends(get_supabase_client)):
    ...
    try:
        # Verificar se o agente existe
        agent_response = supabase.table("agents").select("*").eq("id", str(agent_id)).execute()
        ...
        agent_tools_response = supabase.table("agent_tools").select("tool_id").eq("agent_id", str(agent_id)).execute()
        ...
        for tool_id in tool_ids:
            tool_response = supabase.table("tools").select("*").eq("id", tool_id).execute()
            if len(tool_response.data) > 0:
...
@router.post("/agents/{agent_id}/tools/{tool_id}", response_model=AgentTool, status_code=status.HTTP_201_CREATED)
async def add_tool_to_agent(
    ...
    try:
        ...
        # Verificar se a ferramenta existe
        tool_response = supabase.table("tools").select("*").eq("id", str(tool_id)).execute()
        ...
        existing_response = supabase.table("agent_tools").select("*").eq("agent_id", str(agent_id)).eq("tool_id", str(tool_id)).execute()
...
async def remove_tool_from_agent(agent_id: UUID4, tool_id: UUID4, supabase=Depends(get_supabase_client)):
    """
