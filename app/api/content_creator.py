...
class EmailRequest(BaseModel):
    ...
    key_points: List[str] = Field(..., description="List of key points to include in the email body")
    tone: Optional[str] = Field("professional", description="Tone of the email (professional, friendly, formal, casual, urgent)")
    include_signature: Optional[bool] = Field(True, description="Whether to include a signature")
...
class SocialMediaRequest(BaseModel):
    ...
    platform: str = Field(..., description="Target social media platform (twitter, linkedin, instagram, facebook)")
    tone: Optional[str] = Field("professional", description="Tone of the post (professional, friendly, formal, casual, promotional)")
    include_hashtags: Optional[bool] = Field(True, description="Whether to include hashtags")
    include_call_to_action: Optional[bool] = Field(True, description="Whether to include a call to action")

class ContentStoreRequest(BaseModel):
    action: str = Field(..., description="The action to perform (save, get, list, delete)")
    content_type: str = Field(..., description="The type of content (email, social_media_post, other)")
    content_data: Optional[Dict[str, Any]] = Field(None, description="The content data to save (for save action)")
    content_id: Optional[str] = Field(None, description="The ID of the content to retrieve/delete (for get/delete actions)")
...
@router.post("/social-media", response_model=Dict[str, Any], summary="Create a social media post")
...
@router.post("/content-store", response_model=Dict[str, Any], summary="Manage stored content")
...
@router.post("/prompt", response_model=AgentResponse, summary="Send a prompt to the Content Creator Agent")
async def send_prompt(request: PromptRequest):
    ...
    try:
        ...
        agent_id = request.conversation_id or "content_creator_" + str(hash(request.prompt))[:8]
        if not agent_factory.agent_exists(agent_id):
            agent_factory.create_agent(agent_id, CONTENT_CREATOR_SYSTEM_PROMPT)
