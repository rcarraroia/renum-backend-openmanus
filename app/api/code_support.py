...
class BoilerplateRequest(BaseModel):
    language: str = Field(
        ..., description="Programming language (python, javascript, java)"
    )
    code_type: str = Field(
        ..., description="Type of code to generate (function, class)"
    )
    name: str = Field(..., description="Name of the function or class")
    description: Optional[str] = Field(
        None, description="Description of the function or class"
    )
    params: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of parameter dictionaries"
    )
    return_type: Optional[str] = Field(None, description="Return type of the function")
    methods: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of method dictionaries for classes"
    )
    style: Optional[str] = Field(
        "standard", description="Coding style (standard, google, pep8, etc.)"
    )
...
class TestGeneratorRequest(BaseModel):
    ...
    framework: str = Field(
        ..., description="Testing framework (unittest, pytest, jest, mocha, junit)"
    )
    test_type: str = Field(..., description="Type of test (function, class)")
    module_name: str = Field(
        ..., description="Name of the module containing the code to test"
    )
    function_name: Optional[str] = Field(
        None, description="Name of the function to test (for function tests)"
    )
    class_name: Optional[str] = Field(
        None, description="Name of the class to test (for class tests)"
    )
    methods: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of method dictionaries (for class tests)"
    )
    params: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of parameter dictionaries"
    )
    ...
    test_cases: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of test case dictionaries"
    )
    package_name: Optional[str] = Field(
        None, description="Java package name (for Java tests)"
    )
...
class ProjectStructureRequest(BaseModel):
    ...
    framework: str = Field(
        ..., description="Framework or project type (basic, flask, node, react, maven)"
    )
    ...
    output_dir: Optional[str] = Field(
        None, description="Directory where the project will be generated"
    )
    package_name: Optional[str] = Field(
        None, description="Name of the package (for Java/Node.js projects)"
    )
    group_id: Optional[str] = Field(
        None, description="Group ID (for Java Maven projects)"
    )
...
@router.post(
    "/boilerplate", response_model=Dict[str, Any], summary="Generate code boilerplate"
)
async def generate_boilerplate(
    request: BoilerplateRequest, supabase=Depends(get_supabase_client)
):
    """
...
@router.post(
    ...
    summary="Send a prompt to the Code Support Agent",
)
async def send_prompt(request: PromptRequest):
    ...
    try:
        ...
        agent_id = (
            request.conversation_id or "code_support_" + str(hash(request.prompt))[:8]
        )
        if not agent_factory.agent_exists(agent_id):
            agent_factory.create_agent(agent_id, CODE_SUPPORT_SYSTEM_PROMPT)
