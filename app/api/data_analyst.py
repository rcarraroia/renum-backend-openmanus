...
class DataLoaderRequest(BaseModel):
    source: str = Field(
        ..., description="Path to the data source (file path or connection string)"
    )
    source_type: Optional[str] = Field(
        None, description="Type of the source (csv, json, database)"
    )
    cache: Optional[bool] = Field(True, description="Whether to cache the loaded data")
    query_params: Optional[Dict[str, Any]] = Field(
        None, description="Additional parameters for database queries"
    )
...
class SalesAnalysisRequest(BaseModel):
    action: str = Field(
        ...,
        description="Analysis action to perform (total, growth, highest_growth, etc.)",
    )
    data: List[Dict[str, Any]] = Field(..., description="Sales data to analyze")
    period: Optional[str] = Field(
        None, description="Time period for analysis (day, week, month, quarter, year)"
    )
    ...
    amount_field: Optional[str] = Field(
        "amount", description="Field name for amount values"
    )
    product_field: Optional[str] = Field(
        "product", description="Field name for product values"
    )
    category_field: Optional[str] = Field(
        "category", description="Field name for category values"
    )
...
class FilteringRequest(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(
        None, description="Dictionary of field-value pairs to filter by"
    )
    ...
    aggregate: Optional[Dict[str, str]] = Field(
        None, description="Dictionary of field-aggregation pairs for grouped data"
    )
...
@router.post(
    ...
    summary="Load data from various sources",
)
async def load_data(request: DataLoaderRequest, supabase=Depends(get_supabase_client)):
    """
...
@router.post(
    "/sales-analysis", response_model=Dict[str, Any], summary="Analyze sales data"
)
...
@router.post(
    "/filtering", response_model=Dict[str, Any], summary="Filter and transform data"
)
...
@router.post(
    ...
    summary="Send a prompt to the Data Analyst Agent",
)
async def send_prompt(request: PromptRequest):
    ...
    try:
        ...
        agent_id = (
            request.conversation_id or "data_analyst_" + str(hash(request.prompt))[:8]
        )
        if not agent_factory.agent_exists(agent_id):
            agent_factory.create_agent(agent_id, DATA_ANALYST_SYSTEM_PROMPT)
